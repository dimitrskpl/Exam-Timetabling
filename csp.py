...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints):
        ...
        self.nassigns = 0
        self.checks = 0
        self.weight = dict() #for dom_wdeg
        for var1 in self.variables:
            for var2 in self.variables:
                self.weight[(var1, var2)] = 1
...
def revise(csp, Xi, Xj, removals, checks=0):
    ...
    if not csp.curr_domains[Xi]: #dom(x) is empty
        csp.weight[(Xi, Xj)] += 1
        csp.weight[(Xj, Xi)] += 1
...
def program_constraint(A, a, B, b):
    if a==b: 
        return False  #same day, hour

    if a[0] == b[0] and (A[0] == B[0] or A[2] == B[2] or (A[4] and B[4]) or (A[4] and b[1] - a[1] == 1) or (B[4] and a[1]- b[1] == 1)) :
        return False #same day and (same samester or same teacher or both subjects have lab or A has lab and B after A or B has lab and A after B)

    if A[3] and B[3] and abs(a[0]-b[0]) < 2: 
        return False #difficult subjects and not at least 2 days difference between their exam day

    return True


class programCSP(CSP):
    def __init__(self, fileName):
        ...
        rows = []
        for row in csvreader: #insert each line in a list
            rows.append(row)
        ...
        for row in rows:
            ...
            if row[4] == "TRUE":
                has_lab = True
            variables.append((samester, row[1], row[2], difficult, has_lab))
        
        list1 = []
        for i in range(1, self.total_days+1):
            for j in range(1, self.total_slots+1):
                list1.append((i, j))

        list2 = []
        for i in range(1, self.total_days+1):
            for j in range(1, self.total_slots):
                list2.append((i, j))
        ...
        for var in variables:
            neighbors[var] = [curVar for curVar in variables if curVar != var]

        CSP.__init__(self, variables, domain, neighbors, program_constraint)
            ...
            for i in range(1, self.total_days+1):
                ...
                for j in range(1,self.total_slots+1):
                    for key in assignment:
                        if assignment[key] == (i, j):
                            print(j, " -> ", key)
                            if key[4] == True: #subject has lab
                                new_sub = list(key)
                                new_sub[1] = key[1] + "_lab" #create new subject adding '_lab' at the end of the name
